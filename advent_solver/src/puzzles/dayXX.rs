...
pub async fn solve_a(input_lines: &[String]) -> io::Result<()> {
    println!("Solving Day X, Part A");
    send_data_to_pico(input_lines.to_vec()).await
}

pub async fn solve_b(input_lines: &[String]) -> io::Result<()> {
    println!("Solving Day X, Part B");
    send_data_to_pico(input_lines.to_vec()).await
}
